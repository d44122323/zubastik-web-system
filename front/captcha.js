(function () {
  ...
  async function getCaptcha() {
    const r = await fetch('/api/captcha', { credentials: 'include', cache: 'no-store' });
    if (!r.ok) throw new Error(await r.text() || 'Не удалось получить проверку');
    return r.json();
  }
  ...
  function makeCaptchaBox() {
    ...
    const load = async () => {
      ...
      try {
        ...
        box.querySelector('.site-captcha-question').textContent = 'Ошибка загрузки';
        box.querySelector('.site-captcha-error').textContent = e.message || 'Не удалось получить проверку';
      }
    };
    box._reloadCaptcha = load;
    box.querySelector('.site-captcha-refresh').addEventListener('click', load);
    box.querySelector('.site-captcha-input').addEventListener('input', e => { state.answer = e.target.value.trim(); });
    load();
    return box;
  }

  function authGate() {
    const old = document.querySelector('.site-auth-gate'); if (old) old.remove();
    ...
    const go = () => { location.href = '/?account=patient&record=1'; };
    gate.querySelector('.site-auth-primary').onclick = go;
    gate.querySelector('.site-auth-secondary').onclick = () => { location.href = '/?account=register&record=1'; };
  ...
  function installRegistrationCaptcha() {
    ...
    document.addEventListener('submit', async function (event) {
      ...
      if (!state.token || !answer) {
        box.querySelector('.site-captcha-error').textContent = 'Ответьте на проверку';
        return;
      }
      const payload = {
        ...
        privacyConsent: !!document.getElementById('homePrivacyConsent').checked,
        captchaToken: state.token,
        captchaAnswer: answer
      };
      const msg = document.getElementById('homeAuthMsg');
      if (!payload.privacyConsent) { msg.textContent = 'Для регистрации необходимо согласиться на обработку персональных данных и с Политикой конфиденциальности.'; return; }
      msg.textContent = 'Создание аккаунта...';
      try {
        const r = await fetch('/patient/register', {method:'POST',headers:{'Content-Type':'application/json'},credentials:'include',body:JSON.stringify(payload)});
        if (!r.ok) {
      ...
      } catch (e) { msg.textContent = 'Не удалось создать аккаунт'; await box._reloadCaptcha(); }
    }, true);
  }
  ...
  async function installRequestGuard(form) {
    ...
    document.addEventListener('submit', async function (event) {
      ...
      try {
        const auth = await fetch('/api/patient/me', {credentials:'include',cache:'no-store'});
        if (!auth.ok) { authGate(); return; }
      } catch (_) { authGate(); return; }
      ...
      const actions = document.createElement('div'); actions.className='site-auth-gate-actions';
      actions.innerHTML = '<button type="button" class="site-auth-primary">Отправить заявку</button><button type="button" class="site-auth-secondary">Отмена</button>';
      ...
      windowBox.addEventListener('click', e => { if(e.target===windowBox) close(); });
      actions.children[0].onclick = () => {
        const state = box._captchaState;
        const answer = box.querySelector('.site-captcha-input').value.trim();
        if (!state.token || !answer) { box.querySelector('.site-captcha-error').textContent='Ответьте на проверку'; return; }
        let token = form.querySelector('input[name="captcha_token"]');
        if (!token) { token=document.createElement('input'); token.type='hidden'; token.name='captcha_token'; form.appendChild(token); }
        let answerInput = form.querySelector('input[name="captcha_answer"]');
        if (!answerInput) { answerInput=document.createElement('input'); answerInput.type='hidden'; answerInput.name='captcha_answer'; form.appendChild(answerInput); }
        token.value=state.token; answerInput.value=answer;
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('form').forEach(form => {
      ...
      if (action === '/submit' || /\/submit(?:\?|$)/.test(action)) installRequestGuard(form);
    });
  });
})();
