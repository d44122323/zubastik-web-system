...
function renderDoctors() {
  ...
  document.querySelectorAll(".deactivate-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      changeStatus(Number(btn.dataset.id), "deactivate"),
    );
  });

  document.querySelectorAll(".activate-btn").forEach((btn) => {
    btn.addEventListener("click", () =>
      changeStatus(Number(btn.dataset.id), "activate"),
    );
  });
}
...
function openEdit(id) {
  ...
  document.getElementById("specializationFull").value =
...
async function saveDoctor(event) {
  ...
  const payload = {
    ...
    photo: id
      ? doctors.find((item) => item.id === Number(id))?.photo || ""
      : "",
    ...
    description: document
      ...
      .map((item) => item.trim())
      .filter(Boolean),
    specializationFull: document
      .getElementById("specializationFull")
      .value.trim(),
  };

  const response = await fetch(id ? `/api/doctors/${id}` : "/api/doctors", {
    method: id ? "PUT" : "POST",
  ...
  try {
    ...
    await loadDoctors();
    showMessage(
      `Врач сохранён, но фото не загрузилось: ${error.message}`,
      "error",
    );
    return;
  }
...
async function openAccount(id) {
  ...
  if (!response.ok) {
    showMessage("Не удалось загрузить учётную запись", "error");
    return;
  }

  const account = await response.json();
  document.getElementById("accountLogin").value =
    account.login || `doctor${id}`;
  document.getElementById("accountActive").checked = account.exists
    ? account.isActive
    : true;
...
async function saveAccount(event) {
  ...
  if (response.status === 409) {
    showMessage("Такой логин уже используется", "error");
    return;
  }
  if (!response.ok) {
    const text = await response.text();
...
function openSchedule(id) {
  ...
  scheduleDays.innerHTML =
    '<div class="schedule-loading">Загрузка расписания...</div>';
  document.getElementById("unavailabilityList").innerHTML =
    '<div class="schedule-loading">Загрузка...</div>';
  ...
  fetch(`/api/doctors/${id}/schedule`)
    ...
    .then((schedule) => {
      if (!schedule) return;
      scheduleDays.innerHTML = schedule.days
        .map(
          (day) => `
  ...
  try {
    ...
    list.innerHTML = items.length
      ? items
          ...
          .join("")
      : '<div class="empty-small">Периодов недоступности нет.</div>';
    list
      .querySelectorAll(".delete-unavailability")
      .forEach((btn) =>
        btn.addEventListener("click", () =>
          deleteUnavailability(Number(btn.dataset.id)),
        ),
      );
  } catch (_) {
    document.getElementById("unavailabilityList").innerHTML =
      '<div class="empty-small">Не удалось загрузить периоды.</div>';
  }
}
...
async function addUnavailability(event) {
  ...
  const response = await fetch(
    `/api/doctors/${unavailabilityDoctorId}/unavailability`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
  ...
  if (!response.ok) {
    showMessage("Не удалось добавить период недоступности", "error");
    return;
  }
...
async function deleteUnavailability(id) {
  ...
  const response = await fetch(
    `/api/doctors/${unavailabilityDoctorId}/unavailability/${id}`,
    { method: "DELETE" },
  );
  if (!response.ok) {
    showMessage("Не удалось удалить период", "error");
    return;
  }
  await loadUnavailability(unavailabilityDoctorId);
