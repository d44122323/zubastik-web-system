async function loadDoctor() {
    ...
    try {
        ...
        document.getElementById('doctorSpecializationFull').textContent = doctor.specializationFull;
        document.getElementById('doctorEducation').innerHTML = `<p>${escapeHtml(doctor.education)}</p>`;
        document.getElementById('doctorDescription').innerHTML = (doctor.description || []).map(text => `<p>${escapeHtml(text)}</p>`).join('');

        const bookingUrl = `index.html?doctor=${doctor.id}#contacts`;
        ['doctorRecordTop', 'doctorRecord', 'doctorRecordFooter'].forEach(elementId => {
            const link = document.getElementById(elementId);
            if (link) link.href = bookingUrl;
        });
        initBooking(doctor.id);
    } catch (error) {
        document.getElementById('doctorProfile').innerHTML = '<div class="detail-card"><h2>Врач не найден</h2><p>Вернитесь к списку специалистов.</p></div>';
        console.error(error);
    }
}
...
function initBooking(doctorId) {
    ...
    async function loadAvailability() {
        ...
        try {
            const response = await fetch(`/api/doctors/${doctorId}/availability?date=${encodeURIComponent(dateInput.value)}`);
            const data = await response.json();
            if (!response.ok) throw new Error(data || 'Ошибка загрузки');
            if (!data.isWorking) { message.textContent = `${data.dayName} — выходной`; slots.innerHTML = '<div class="slot-empty">В этот день врач не принимает.</div>'; return; }
            const free = (data.slots || []).filter(slot => slot.available);
            message.textContent = `${data.dayName}, ${data.startTime}–${data.endTime}`;
            if (!free.length) { slots.innerHTML = '<div class="slot-empty">Свободных мест на эту дату нет.</div>'; return; }
            free.forEach(slot => {
                const button = document.createElement('button');
                button.type = 'button'; button.className = 'slot-btn'; button.textContent = slot.time;
            ...
            console.error(error); message.textContent = 'Не удалось загрузить расписание.';
        }
    }
