async function loadDoctors() {
    ...
    try {
        ...
        if (homepage) {
            ...
            if (doctors.length === 0) {
                homepage.innerHTML = '<p class="specialists-loading">Специалисты пока не добавлены.</p>';
            ...
            doctors.forEach(doctor => {
                ...
                const description = Array.isArray(doctor.description) ? doctor.description : [];
            ...
            let page = 0;
            const getPerPage = () => window.innerWidth <= 768 ? 1 : window.innerWidth <= 1100 ? 2 : 3;
            const renderCarousel = () => {
                ...
                Array.from(homepage.children).forEach((card, index) => {
                    ...
                    card.style.display = index >= offset && index < offset + perPage ? 'flex' : 'none';
                });
                if (prev) { prev.hidden = pages <= 1; prev.disabled = page === 0; }
                if (next) { next.hidden = pages <= 1; next.disabled = page === pages - 1; }
            };
            ...
            next?.addEventListener('click', () => {
                const perPage = getPerPage();
                if (page < Math.ceil(doctors.length / perPage) - 1) { page++; renderCarousel(); }
            });
        ...
        container.querySelectorAll('.toggle-btn').forEach(button => {
            button.addEventListener('click', () => {
                const text = button.closest('.doctor-main').querySelector('.doctor-text');
                text.classList.toggle('open');
                button.textContent = text.classList.contains('open') ? 'Свернуть' : 'Читать полностью';
            });
        });
    } catch (error) {
        console.error('Ошибка загрузки специалистов:', error);
        if (homepage) {
            homepage.innerHTML = '<p class="specialists-loading">Не удалось загрузить специалистов.</p>';
        }
    }
}
