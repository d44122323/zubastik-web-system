...
function openServiceModal(id) {
  ...
  box.innerHTML = services.length
    ? services
        .map(
          (s) =>
            `<div class="service-option"><label><input type="checkbox" value="${s.id}" data-name="${esc(s.name)}" data-price="${s.price}" ${selected.has(s.name) ? "checked" : ""}> <span>${esc(s.name)}</span></label><span class="service-option-price">${Number(s.price).toLocaleString("ru-RU")} ₽</span></div>`,
        )
        .join("")
    : '<div class="empty">Услуги не загружены.</div>';
  updateServiceTotal();
...
function updateServiceTotal() {
  let total = 0;
  document
    .querySelectorAll("#serviceOptions input[type=checkbox]:checked")
...
async function saveServices() {
  ...
  const b = document.getElementById("serviceSave");
  b.disabled = true;
  try {
    const r = await fetch("/api/doctor/requests/" + currentServiceRequestId, {
      method: "PUT",
      credentials: "include",
    ...
    const i = allRequests.findIndex(
      (x) => Number(x.id) === currentServiceRequestId,
    );
    if (i >= 0) allRequests[i] = updated;
    closeServiceModal();
    applyFilters();
...
function render(items) {
  ...
  if (!items.length) {
    box.innerHTML =
      '<div class="empty">Заявок по выбранным условиям нет.</div>';
    return;
  }
  box.innerHTML = items
    .map((x) => {
      ...
      if (x.status === "Новая")
        actions =
          '<button class="primary" data-action="Подтверждена" data-id="' +
          x.id +
          '">Подтвердить</button><button class="danger" data-action="Отменена" data-id="' +
          x.id +
          '">Отклонить</button>';
      if (x.status === "Подтверждена")
        actions =
          '<a class="primary action-link" href="/doctor/medical-record.html?appointment_id=' +
          x.id +
          '">Завершить приём</a><button class="telegram-send" data-tg-id="' +
          x.id +
          '">📨 Telegram</button><button class="danger" data-action="Отменена" data-id="' +
          x.id +
          '">Отменить</button>';
      return (
        '<article class="request"><div><h3>' +
        esc(x.name) +
        '</h3><div class="meta">' +
        esc(x.services || "Услуга не указана") +
        " " +
        (x.services
          ? '<button type="button" class="service-edit" data-service-id="' +
            x.id +
        ...
        '</div><div class="meta">' +
        esc(x.appointment_date || "Дата не назначена") +
        " · " +
        esc(x.appointment_time || "Время не назначено") +
        '</div><div class="meta">' +
        esc(x.phone) +
  ...
  box
    ...
    .forEach(
      (b) => (b.onclick = () => changeStatus(b.dataset.id, b.dataset.action)),
    );
  box.querySelectorAll("[data-service-id]").forEach(
    (b) =>
      (b.onclick = (e) => {
        e.stopPropagation();
        openServiceModal(b.dataset.serviceId);
      }),
  );
  box.querySelectorAll("[data-tg-id]").forEach(
    (b) =>
      (b.onclick = async () => {
        b.disabled = true;
        try {
          const r = await fetch(
            "/api/doctor/telegram/request/" + b.dataset.tgId,
...
document
  ...
  .addEventListener("change", updateServiceTotal);
...
document.querySelectorAll("#tabs button[data-status]").forEach(
  (b) =>
    (b.onclick = () => {
...
document.getElementById("clear").onclick = () => {
  ...
  document
    ...
    .forEach((x) => x.classList.remove("active"));
  document
    .querySelector('#tabs button[data-status=""]')
